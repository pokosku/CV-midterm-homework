...
cv::Rect bbox_from_clustering(const std::vector<cv::Point2f>& points, cv::Size frame_size) {
    ...
    // Distance threshold for neighbors, we choose 40 pixels
    float dist_threshold = 40.0f; 
    
    // We split the points into clusters, following the condition of euclidean distance between two points and the threshold
    ...
    // We compute the most crowded cluster
    ...
    // Now we extract the points from the best cluster
    ...
    if (best_cluster_points.size() < 4) return cv::Rect(0, 0, 0, 0);

    // Output the bounding box of the clustered best points
